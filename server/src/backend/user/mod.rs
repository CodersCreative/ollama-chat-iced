...
DEFINE FIELD IF NOT EXISTS name ON TABLE {0} TYPE string DEFAULT $auth.name;
DEFINE FIELD IF NOT EXISTS email ON TABLE {0} TYPE string DEFAULT $auth.email;
...
DEFINE TABLE IF NOT EXISTS {1} SCHEMAFULL PERMISSIONS FULL;
