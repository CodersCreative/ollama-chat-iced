        ...
        match self {
            ...
            Self::FilesSelected(paths) => {
                ...
                for path in paths {
                    ...
                    let mut builder = B64FileDataBuilder::default();

                    let file =
                        if IMAGE_FORMATS.contains(&file.extension().unwrap().to_str().unwrap()) {
                            match convert_image_to_b64(file) {
                                Ok(x) => builder
                                    .b64data(x)
                            ...
                            match convert_file_to_b64(file) {
                                Ok(x) => builder
                                    .b64data(x)
                ...
                Task::batch(files.into_iter().map(|file| {
                    Task::future(async move {
                            ...
                            .await
                        {
                            Ok(x) => Message::HomePaneView(HomePaneViewMessage::Chats(
                                id,
                                ChatsViewMessage::FileUploaded(ViewFile::from(B64File {
        ...
        let images = lazy(&message.files, |files| {
            container(
                scrollable::Scrollable::new(
                    row(files.iter().map(|x| {
                        let inner: Element<'_, Message> = match &x.data {
                            ViewFileType::Image(x) => image(image::Handle::from_bytes(x.clone()))
                                .height(200)
                                .into(),
                            _ => row![text(x.filename.clone())].into(),
                        };
                        button(inner)
                            .on_press(Message::Cache(CacheMessage::ExpandFile(x.clone())))
