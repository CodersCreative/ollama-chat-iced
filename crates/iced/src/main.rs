...
use crate::{
    font::{BODY_SIZE, HEADER_SIZE, SUB_HEADING_SIZE, get_iced_font},
    pages::{
        Pages,
        ...
        home::{
            ...
            panes::{
                data::{
                    HomePaneSharedData, MessageMk, ModelsData, OptionsData, PromptsData, ViewFile,
                    ViewFileType,
                },
                view::{
...
impl CacheMessage {
    pub fn handle(self, app: &mut Application) -> Task<Message> {
        match self {
            ...
            Self::ExpandFile(x) => {
                app.cache.expanded_file = Some(x);
            }
            Self::CloseExpandedFile => {
                app.cache.expanded_file = None;
            }
            Self::SetSettings(x) => {
...
impl Application {
    ...
    pub fn view<'a>(&'a self, window_id: window::Id) -> Element<'a, Message> {
        ...
        if let Some(data) = &self.cache.expanded_file {
            let size = self.windows.get(&window_id).unwrap().size.clone();
            let inner: Element<'a, Message> = match &data.data {
            ...
            body = stack![
                ...
                .on_press(Message::Cache(CacheMessage::CloseExpandedFile))
            ]
            .into()
        }
