...
impl Chat{
    ...
    pub fn view(&self, app : &ChatApp) -> Element<Message>{
        ...
        let title = button(
            text(self.get_title()).align_x(Horizontal::Center).align_y(Vertical::Center).width(Length::Fill).size(20)
        )
        .style(style)
        .on_press(Message::Chats(ChatsMessage::ChangeChat(self.get_id().clone()), app.panes.last_chat))
