...
impl Download {
    ...
    pub fn progress(
        &mut self,
        new_progress: Result<DownloadProgress, String>,
    ) {
    ...
    pub fn view(&self, app : &ChatApp) -> Element<Message> {
        ...
        let info = Self::txt(format!("Downloading... {per:.2}%"), app.theme().palette().danger);
        ...
        container(button(column![
            ...
            info,
            message
        ]).style(style::button::transparent_text).on_press(Message::StopPull(self.id))).padding(10).into()
    }
}
...
pub fn download_stream(model : String, ollama : Arc<Mutex<Ollama>>) -> impl Stream<Item = Result<DownloadProgress, String>>{
    try_channel(1, move |mut output| async move{
        let ollama = ollama.lock().await;
        let mut y = ollama.pull_model_stream(model, false).await.map_err(|x| x.to_string())?;
        let mut total = 1;
        let mut completed = 0;
        let _ = output.send(DownloadProgress::Downloading (0.0, String::new())).await;
        ...
        while let Some(status) = y.next().await{
            ...
            let _ = output.send(DownloadProgress::Downloading (completed as f32 / total as f32 * 100.0, status.message)).await;
        }
